...
func setSelfhostedEnv(t *testing.T) string {
	...
	t.Setenv("YASAKU_MODE", "selfhosted")
	t.Setenv("YASAKU_DB_DRIVER", "sqlite")
	t.Setenv("YASAKU_DB_DSN", filepath.Join(dir, "alt.db"))
	t.Setenv("YASAKU_HTTP_ADDR", "127.0.0.1:0")
	t.Setenv("YASAKU_HTTP_BASEURL", "http://127.0.0.1")
	t.Setenv("YASAKU_GENESIS_EMAIL", "root@example.com")
	t.Setenv("YASAKU_GENESIS_PASSWORD", "hunter2")
	t.Setenv("YASAKU_SESSION_PATH", sessPath)
	t.Setenv("YASAKU_MAIL_DRIVER", "console")
	t.Setenv("YASAKU_MAIL_FROM", "no-reply@example.com")
	return sessPath
}
